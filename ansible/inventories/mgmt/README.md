...
| 目录/文件 | 说明 |
|-----------|------|
| `hosts.yml` | 分组：`mgmt` → `mgmt_hub` → `hub-01`；`wireguard_peers` → `ci-01`；`jumpserver_assets` → `hub-01` |
...
| `group_vars/all/jumpserver_asset.yml` | JumpServer 资产纳管 — Linux 用户侧 SSOT |
| `group_vars/jumpserver_assets/main.yml` | 组内主机 `jumpserver_asset.enabled: true` |
...
| `ansible/playbooks/jumpserver-asset-prep.yml` | 资产纳管：jump_ops 系统用户准备 |
