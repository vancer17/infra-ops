...
.PHONY: help setup lint syntax inventory inventory-mgmt ci \
        ...
        build-gateway-images save-gateway-images \
...
help:
	...
	@echo "  Asset prep: make stage-jumpserver-asset-preflight JUMPSERVER_ASSET_LIMIT=hub-01"
	@echo "              make jumpserver-asset-prep JUMPSERVER_ASSET_LIMIT=hub-01"
	...
	@echo "            make build-gateway-images / make save-gateway-images (本地 tag，非轩辕 push)"
	@echo ""
...
stage-g1-nginx-preflight:
	bash "$(SCRIPTS_MGMT)/stage-g1-nginx-preflight.sh"
	@echo "[make] stage-g1-nginx-preflight OK"

stage-g2-preflight:
	bash "$(SCRIPTS_MGMT)/stage-g2-preflight.sh"
	@echo "[make] stage-g2-preflight OK"
...
JUMPSERVER_ASSET_LIMIT ?= hub-01

stage-jumpserver-asset-preflight:
	ANSIBLE_LIMIT="$(JUMPSERVER_ASSET_LIMIT)" bash "$(SCRIPTS_MGMT)/stage-jumpserver-asset-preflight.sh"
	@echo "[make] stage-jumpserver-asset-preflight OK (limit=$(JUMPSERVER_ASSET_LIMIT))"

jumpserver-asset-prep-preflight: stage-jumpserver-asset-preflight
	@echo "[make] jumpserver-asset-prep-preflight OK (alias of stage-jumpserver-asset-preflight)"

jumpserver-asset-prep:
	bash "$(SCRIPTS_MGMT)/jumpserver-asset-prep.sh" all "$(JUMPSERVER_ASSET_LIMIT)"
	@echo "[make] jumpserver-asset-prep OK (limit=$(JUMPSERVER_ASSET_LIMIT))"

apply-hub-deploy-sudo:
	bash "$(SCRIPTS_MGMT)/apply-hub-deploy-sudo.sh"

apply-ci-wireguard-sudo:
	bash "$(SCRIPTS_MGMT)/apply-ci-wireguard-sudo.sh"
...
save-gateway-images:
	bash "$(REPO_ROOT)/scripts/docker/build-gateway-images.sh" save
	@echo "[make] save-gateway-images OK — set gateway.images.delivery=bundle for cross-host deploy"
